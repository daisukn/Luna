...
Application::Application()
    ...
    , mActiveStage(nullptr)
    , mStageTransition(STAGE_TITLE)
    , mIsPause(false)
{
...
void Application::ProcessInput(){
    ...
    // Stageに渡す
    mActiveStage->ProcessInput(state);
}
...
void Application::LoadData()
{
    ...
    // ステージ読み込み
    mStageTransition = STAGE_TITLE;
    mActiveStage = std::make_unique<TitleStage>(this);
}
...
void Application::UpdateGame()
{
    ...
    mActiveStage->UpdateStage();
    if(mActiveStage->GetQuitStage())
    {
        TransitionStage();
    }
    // Actorsメイン呼び出し
...
void Application::TransitionStage()
{
    ...
    switch (mStageTransition)
    {
        case STAGE_TITLE:
            mStageTransition = STAGE_CLOUD;
            mActiveStage = std::make_unique<CloudStage>(this);
            break;
